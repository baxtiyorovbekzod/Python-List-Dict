def get_min_max_age_users(users):
    min_user = users[0]
    max_user = users[0]

    for user in users:
        if user["age"] < min_user["age"]:
            min_user = user
        if user["age"] > max_user["age"]:
            max_user = user
...
result = get_min_max_age_users(users)

print("Eng yosh foydalanuvchi:", result[0])
print("Eng katta foydalanuvchi:", result[1])
